...
export async function GET(request: Request) {
  try {
    ...
    // 6. is_curious 필드 추가 (deviceHash 또는 로그인 사용자 있을 때)
    let finalData = data
    if (deviceHash || currentUserId) {
      ...
      if (reportIdsForCurious.length > 0) {
        ...
        if (deviceHash) {
          ...
          if (!curiousCheckError && curiousCheckData) {
            curiousReportIds = [...curiousReportIds, ...curiousCheckData.map(r => r.report_id)]
          }
        }

        // user_id로도 체크 (로그인 사용자가 다른 기기에서 누른 경우 포함)
        if (currentUserId) {
          const { data: curiousCheckByUser, error: curiousCheckByUserError } = await supabase
            .from('report_curious')
            .select('report_id')
            .eq('user_id', currentUserId)
            .in('report_id', reportIdsForCurious)

          if (!curiousCheckByUserError && curiousCheckByUser) {
            curiousReportIds = [...curiousReportIds, ...curiousCheckByUser.map(r => r.report_id)]
          }
        }
