...
interface IssueDetailClientProps {
  ...
  initialChatState: ChatRoomState | null
}

export function IssueDetailClient({ issue, articles, initialChatState }: IssueDetailClientProps) {
  const router = useRouter()

  const { user, loading: authLoading, signInWithGoogle } = useAuth()
  const [chatState, setChatState] = useState<ChatRoomState | null>(initialChatState)
  ...
  const poll = issue.poll?.[0] ? {
    id: issue.poll[0].id,
    question: issue.poll[0].question,
    options: issue.poll[0].options?.map((opt: any) => ({
      id: opt.id,
      label: opt.label,
  ...
  useEffect(() => {
    ...
    async function refreshChatState() {
      try {
        setChatLoading(true)
        setChatError(null)
        const state = await fetchChatRoomState(String(issue.id))
        if (!cancelled) {
          setChatState(state)
        }
      } catch (err) {
        console.error('Failed to refresh chat room state:', err)
        if (!cancelled) {
    ...
    // 30초마다 채팅방 상태 갱신
    const interval = setInterval(refreshChatState, 30000)
