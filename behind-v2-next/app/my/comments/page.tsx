...
export default function MyCommentsPage() {
  ...
  const { data: commentsData, isLoading, error, fetch: fetchWithRetry } = useFetchWithRetry<CommentsResponse>()
  ...
  }, [user, loading, page, fetchWithRetry])
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      ...
      {/* 댓글 목록 */}
      {!error && !isLoading && commentsData && (!commentsData.comments || commentsData.comments.length === 0) ? (
        <Card className="p-12 text-center">
          <p className="text-gray-500">작성한 댓글이 없습니다.</p>
        </Card>
      ) : !error && !isLoading && commentsData ? (
        <>
          <div className="space-y-4">
          ...
          {commentsData.pagination.totalPages > 1 && (
            <div className="flex justify-center gap-2 mt-8">
              ...
              <Button
                variant="outline"
                size="sm"
                disabled={page === commentsData.pagination.totalPages || isLoading}
                onClick={() => setPage(page + 1)}
              >
                다음
              </Button>
