 ...
 * 제보된 이슈 조회
 */
export async function fetchReportedIssues(deviceHash?: string) {
  const supabase = getSupabaseClient()
  ...
  // is_curious 필드 추가 (deviceHash 있을 때만)
  if (deviceHash) {
    ...
    if (reportIds.length > 0) {
      ...
      if (!curiousError && curiousData) {
        const curiousSet = new Set(curiousData.map(r => r.report_id))
 ...
 * 메인 페이지 전체 데이터 병렬 조회
 */
export async function fetchLandingPageData(deviceHash?: string) {
  const [
    activeIssues,
    pollIssues,
    pastIssues,
    ...
    fetchPastIssues(5),
    fetchTrendingIssues(),
    fetchReportedIssues(deviceHash),
    fetchBannerIssues(),
    fetchCarouselIssues()
  ])
